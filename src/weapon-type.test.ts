...
describe('WeaponTypeSchema invalid cases', () => {
  it('fails when versatile without both damage entries', () => {
    ...
    expect(() => WeaponTypeSchema.parse(data)).toThrow(/requires both oneHanded AND twoHanded/);
  });
  ...
  it('fails when two-handed property includes oneHanded damage', () => {
    ...
    expect(() => WeaponTypeSchema.parse(data)).toThrow(/must not include oneHanded damage/);
  });
...
describe('WeaponTypeIndex and WeaponRange enums', () => {
  it('accepts valid weapon type index', () => {
    expect(() => WeaponTypeIndex.parse('dagger')).not.toThrow();
  });

  it('rejects invalid weapon type index', () => {
    expect(() => WeaponTypeIndex.parse('laser')).toThrow();
  });
